    ...
    */
    processTransfer(senderId, recipientsId, amount, transactionType = "CR") {
        return new Promise((resolve, reject) => {
            this.usersBalance(recipientsId).then(oldBalance => { //get balance
                // const newBalance = (oldBalance + amount);
                // this.updateBalance(recipientsId, newBalance);
                if (transactionType === "CR") { //if crediting wallet
                    return this.creditTransactions(senderId, recipientsId, amount, oldBalance);
                } else if (transactionType === "DR") {
                    if (oldBalance >= amount) {
                        return this.debitTransactions(senderId, recipientsId, amount, oldBalance);
                    } else {
    ...
    updateBalance(userId, amount) {
        ...
        return new Promise((resolve, reject) => {
            db.User.update({ balance: amount }, { where: { id: userId } }).then(user => {
                console.log(user);
                resolve(user);
            }).catch(err => {
                reject(err);
     ...
     */
    debitTransactions(userId, recipientsId, amount, oldBalance, narration = "", log = "") {
        return new Promise((resolve, reject) => {
            db.Transaction.create({
                ...
                this.updateBalance(userId, (oldBalance - amount)).then(res => {
                    resolve(transaction);
     ...
     */
    creditTransactions(userId, recipientsId, amount, oldBalance, narration = "", log = "") {
        return new Promise((resolve, reject) => {
            db.Transaction.create({
                amount: amount,
                UserId: userId,// sender
                ...
                // console.log(updated);
                this.updateBalance(recipientsId, amount + oldBalance).then(res => {
                    // console.log(res);
                    resolve(transaction);
                }).catch(err => reject(err));
            }).catch(err => reject(err));
