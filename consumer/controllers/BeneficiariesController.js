...
class BeneficiariesController {
  ...
  static async createUser(req, res, next) {
    try {
      ...
      db.User.findOne({ where: { email: req.body.email, phone: req.body.phone } }).then(user => {
        if (user !== null) {
  ...
  static async updatedUser(req, res) {
    ...
    try {
      const updateUser = await BeneficiariesServices.updateUser(id, alteredUser);
      if (!updateUser) {
  ...
  static async createComplaint(req, res) {
    ...
    if (validation.fails()) {
      ...
      const beneficiary_exist = await BeneficiariesServices.checkBeneficiary(data.beneficiaryId);
      if (beneficiary_exist) {
        const newComplaint = {
          BeneficiaryId: data.beneficiaryId,
          report: data.report
        }
        const complaint = await BeneficiariesServices.createComplaint(newComplaint)
  ...
  static async resolveComplaint(req, res) {
    ...
    if (validation.fails()) {
      ...
      const complaint_exist = await BeneficiariesServices.checkComplaint(data.complaintId)
      if (complaint_exist) {
  ...
  static async getComplaintsByBeneficiary(req, res) {
    ...
    util.setSuccess(200, 'Complaints Retrieved', { complaints: docs, current_page: options.page, pages: pages, total: total, nextPage: nextPage, prevPage: prevPage })
    return util.send(res);
  }

  static async getBeneficiaryUserWallet(req, res) {
    const beneficiary = req.params.beneficiary;
    const beneficiary_exist = await db.User.findOne({ where: { id: beneficiary }, include: ['Wallet'] })
    const campaigns = await db.Beneficiaries.findAll({ where: { UserId: beneficiary }, include: ['Campaign'] })
    if (beneficiary_exist) {
      util.setSuccess(200, 'User Object.', { "user": beneficiary_exist, "associatedCampaigns": campaigns })
      return util.send(res);
