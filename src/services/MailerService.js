...
class MailerService {
  ...
  sendInvite(to, token, campaign, ngo, exist, message, link) {
    ...
    <div>
      ...
      <p>${
        exist
          ? `If you want to login to confirm access, please click on the following link: ${link}/email-verification/?confirmationCode=${token}&campaign_id=${campaign.id}`
          : `If you want to create an account, please click on the following link: ${link}/email-verification/?confirmationCode=${token}&campaign_id=${campaign.id}`
      }</p>
      <p>${message}</p>
      <p>Enjoy!</p>
      <p>Best,</p>
