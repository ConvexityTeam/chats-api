...
const transferToQueue = amqp_1.default.declareQueue('transferTo', {
  durable: true,
});
...
class CashForWorkController {
  static async newTask(req, res) {
    try {
      ...
      if (campaignExist.Jobs.length) {
        const names = campaignExist.Jobs.map((element) => String(element.name).toLowerCase());
  ...
  static async getAllCashForWork(req, res) {
    ...
    cashforworks.forEach((cashforwork) => {
      ...
      const completed = jobs.filter((element) => element.status === 'fulfilled');
  ...
  static async addWorkersToTask(req, res) {
    try {
      ...
      const usersIds = data.users.map((element) => element.UserId);
      ...
      if (task.AssociatedWorkers) {
        const addedUsers = task.AssociatedWorkers.map((element) => element.UserId);

        const dd = addedUsers.some((el) => usersIds.includes(el));
  ...
  static async submitProgress(req, res) {
    ...
    form.parse(req, async (err, fields, files) => {
      ...
      const recordIds = records.map((element) => element.id);
      ...
      files.images.forEach(async (image) => {
        ...
        uploadFilePromises.push(
          uploadFile(
            image,
            `pge-${environ}-${fields.taskId}${i += 1}.${ext}`,
      ...
      if (workerRecord.type === 'supervisor') {
        const tasks = await db.Tasks.findByPk(fields.taskId);
  ...
  static async payWages(req, res) {
    try {
      ...
      const userIds = taskExist.AssociatedWorkers.map((el) => el.UserId);
  ...
  static async getAllCashForWorkTaskFieldAgent(req, res) {
    ...
    try {
      ...
      tasks.Jobs.forEach((data) => {
        const dataCopy = { ...data };
        dataCopy.dataValues.OrganisationId = tasks.OrganisationId;
      });
      Response.setSuccess(200, 'Cash for work task retrieved', tasks);
      return Response.send(res);
    } catch (error) {
