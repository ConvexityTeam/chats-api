...
class ComplaintController {
  ...
  static async getCampaignConplaint(req, res) {
    try {
      const complaint = await ComplaintService.getComplaint(req.params.complaint_id);
      Response.setSuccess(HttpStatusCode.STATUS_OK, 'Campaign Complaint.', complaint);
      return Response.send(res);
    } catch (error) {
      console.log(error)
      Response.setError(HttpStatusCode.STATUS_INTERNAL_SERVER_ERROR, `Internal server error. Contact support.`);
  ...
  static async resolveCampaignConplaint(req, res) {
    try {
      await ComplaintService.updateComplaint(req.params.complaint_id, {status: 'resolved'});
      const complaint = req.complaint.toJSON();
      complaint.status = 'resolved';
      Response.setSuccess(HttpStatusCode.STATUS_OK, 'Campaign Complaint Resolved.', complaint);
      return Response.send(res);
    } catch (error) {
      console.log(error)
      Response.setError(HttpStatusCode.STATUS_INTERNAL_SERVER_ERROR, `Internal server error. Contact support.`);
