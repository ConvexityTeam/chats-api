...
class UtilController {
  static async getCountries(req, res) {
    try {
      ...
      Response.setSuccess(HttpStatusCode.STATUS_OK, 'Countries data.', countries);
  ...
  static async resolveAccountNumber(req, res) {
    try {
      const {account_number, bank_code} = SanitizeObject(req.query, ['account_number', 'bank_code']);
      const response = await PaystackService.resolveAccount(account_number, bank_code);
