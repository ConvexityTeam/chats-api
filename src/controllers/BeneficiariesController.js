...
class BeneficiariesController {
          ...
          bcrypt.genSalt(10, (err, salt) => {
            bcrypt.hash(req.body.password, salt).then((hash) => {
              ...
              return db.User.create({
                ...
                .then((the_user) => {
                  util.setSuccess(201, 'Account Successfully Created', the_user.id);
                  return util.send(res);
                })
                .catch((error) => {
  ...
  static async createComplaint(req, res) {
    ...
    if (beneficiary_exist) {
      ...
      await BeneficiaryService.createComplaint(
        newComplaint,
      );
  ...
  static async getProfile(req, res) {
    try {
      ...
      const beneficiary = await BeneficiaryService.beneficiaryProfile(
        req.user.id,
      );

      // const Wallets = beneficiary.Wallets.map(wallet => {
      //   total_wallet_balance += wallet.balance;
      //   // total_wallet_spent += wallet.SentTransactions.map(tx =>
      //    tx.amount).reduce((a, b) => a + b, 0);
      //   // total_wallet_received += wallet.ReceivedTransactions.map
      //   (tx => tx.amount).reduce((a, b) => a + b, 0);
      //   const w = wallet.toObject();
      ...
      beneficiary.Wallets.forEach((async (wallet) => {
      ...
      const beneficiaryObject = beneficiary.toObject();
        ...
        for (let i = 0; i < beneficiaries.length; i += 1) {
          if (
            parseInt(moment().format('YYYY')
                - moment(beneficiaries[i].dob).format('YYYY'), 10) >= 18
            && parseInt(moment().format('YYYY')
                - moment(beneficiaries[i].dob).format('YYYY'), 10) <= 29
          ) {
            eighteenTo29 += 1;
          }
          if (
            parseInt(moment().format('YYYY')
                - moment(beneficiaries[i].dob).format('YYYY'), 10) >= 30
            && parseInt(moment().format('YYYY')
                - moment(beneficiaries[i].dob).format('YYYY'), 10) <= 41
          ) {
            thirtyTo41 += 1;
          }
          if (
            parseInt(moment().format('YYYY')
                - moment(beneficiaries[i].dob).format('YYYY'), 10) >= 42
            && parseInt(moment().format('YYYY')
                - moment(beneficiaries[i].dob).format('YYYY'), 10) <= 53
          ) {
            forty2To53 += 1;
          }
          if (
            parseInt(moment().format('YYYY')
                - moment(beneficiaries[i].dob).format('YYYY'), 10) >= 54
            && parseInt(moment().format('YYYY')
                - moment(beneficiaries[i].dob).format('YYYY'), 10) <= 65
          ) {
            fifty4To65 += 1;
          }
          if (
            parseInt(moment().format('YYYY')
                - moment(beneficiaries[i].dob).format('YYYY'), 10) >= 66
          ) {
            sixty6Up += 1;
          }
        }
      ...
      if (beneficiaries.length > 0) {
        beneficiaries.forEach((beneficiary) => {
          if (beneficiary.location.includes('state')) {
            const parsedJson = JSON.parse(beneficiary.location);
  ...
  static async beneficiariesTotalBalance(req, res) {
    try {
      ...
      walletBalance.forEach((balance) => {
        if (parseInt(balance, 10) >= 0 && parseInt(balance, 10) <= 100000) {
          zeroTo100k += 1;
        }
        if (parseInt(balance, 10) >= 100001 && parseInt(balance, 10) <= 200000) {
          hundredKTo200K += 1;
        }
        if (parseInt(balance, 10) >= 200001 && parseInt(balance, 10) <= 300000) {
          twoHundredKTo300K += 1;
        }
        if (parseInt(balance, 10) >= 300001 && parseInt(balance, 10) <= 400000) {
          threeHundredKTo400K += 1;
        }
        if (parseInt(balance, 10) >= 400001 && parseInt(balance, 10) <= 500000) {
          fourHundredKTo500K += 1;
        }
        if (parseInt(balance, 10) >= 500001 && parseInt(balance, 10) <= 600000) {
          fiveHundredKTo600K += 1;
        }
        if (parseInt(balance, 10) >= 600001 && parseInt(balance, 10) <= 700000) {
          sixHundredKTo700K += 1;
        }
        if (parseInt(balance, 10) >= 700001 && parseInt(balance, 10) <= 800000) {
          sevenHundredKTo800K += 1;
        }
        if (parseInt(balance, 10) >= 800001 && parseInt(balance, 10) <= 900000) {
          eightHundredKTo900K += 1;
        }
        if (parseInt(balance, 10) >= 900001 && parseInt(balance, 10) <= 1000000) {
          nineHundredKToOneMill += 1;
        }
      });
    ...
    try {
      ...
      transactions.rows.forEach(async (transaction) => {
        ...
        if (transaction.narration === 'Approve Beneficiary Funding') {
          ...
          transactionCopy.dataValues.narration = `Payment from (${
            ngo.name || ngo.email
          })`;

          transactionCopy.dataValues.transaction_type = 'credit';
        }
        if (transaction.narration === 'Vendor Order') {
          const vendor = await UserService.getAUser(transaction.VendorId);
          transactionCopy.dataValues.narration = `Payment to (${
            `${vendor.first_name} ${vendor.last_name}`
          })`;
          transactionCopy.dataValues.transaction_type = 'debit';
        }
        if (
        ...
        if (transaction.dataValues.ReceiverWallet === null) {
          delete transactionCopy.dataValues.ReceiverWallet;
        }
        if (transaction.dataValues.SenderWallet === null) {
          delete transactionCopy.dataValues.SenderWallet;
        }
      });
      ...
      transactions.rows.forEach((transaction) => {
        ...
        if (typeof transaction.dataValues.ReceiverWallet !== 'undefined') {
          transactionCopy.dataValues.BlockchainXp_Link = `https://testnet.bscscan.com/token/0xa31d8a40a2127babad4935163ff7ce0bbd42a377?a=
      ...
      const periods = transactions.rows.map((each_period) => moment(each_period.createdAt).format('ddd'));
