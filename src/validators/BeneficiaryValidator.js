...
class BeneficiaryValidator extends BaseValidator {
  static ApprovedBeneficiary = [
    ...
    body('approved')
      .custom((approved, { req }) => new Promise((resolve) => {
        if (typeof req.body.rejected === 'undefined' && typeof approved === 'undefined') {
          throw new Error('Either approval or rejection state is required.');
        }
        resolve(true);
      }))
      .bail()
    ...
    body('rejected')
      .custom((rejected, { req }) => new Promise((resolve) => {
        if (typeof req.body.approved === 'undefined' && typeof rejected === 'undefined') {
          throw new Error('Either rejection or approval state is required.');
        }
        resolve(true);
      }))
      .bail()
  ...
  static async validateSelfRegister(req, res, next) {
    ...
    form.parse(req, async (err, fields, files) => {
      if (err) {
        return res.status(HttpStatusCode.STATUS_BAD_REQUEST).json({
          status: 'error',
          message: 'Form parsing failed. Please try again.',
        });
      }
      ...
      if (validation.fails()) {
        return res.status(HttpStatusCode.STATUS_UNPROCESSABLE_ENTITY).json({
          status: 'error',
  ...
  static async IsCampaignBeneficiary(req, res, next) {
    try {
      const campaignId = req.params.campaign_id || req.body.campaign_id || '';
      const beneficiaryId = req.params.beneficiary_id || req.body.beneficiary_id || req.user.id;

      if (!campaignId.trim() || !beneficiaryId.trim()) {
  ...
  static async NotCampaignBeneficiary(req, res, next) {
    try {
      ...
      CampaignService.campaignBeneficiaryExists(campaignId, beneficiaryId);
  ...
  static async BeneficiaryExists(req, res, next) {
    try {
      const organisationId = req.organisation.id || req.params.organisation_id
