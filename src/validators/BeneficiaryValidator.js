...
class BeneficiaryValidator extends BaseValidator {
      ...
      .custom(
        (approved, {req}) =>
          new Promise((resolve, reject) => {
            if (
              typeof req.body.rejected == 'undefined' &&
  ...
  static ageGroup() {
    return [
      param('ageGroup')
        .isNumeric()
        .withMessage('gender must be numeric')
        .notEmpty()
        .withMessage('task progress Id must not be empty.')
    ];
  }
    ...
    form.parse(req, (err, fields, files) => {
      ...
      if (err) {
        Response.setError(
          ...
          'Form parsing failed. Please try again.'
        );
        Response.send(res);
        return;
      }
  ...
  static async IsCampaignBeneficiary(req, res, next) {
    try {
      const campaignId =
        req.params.campaign_id || req.body.campaign_id || campaign.id;
      const beneficiaryId =
        req.params.beneficiary_id || req.body.beneficiary_id || req.user.id;

      if (!campaignId.trim()) {
        Response.setError(
          HttpStatusCode.STATUS_BAD_REQUEST,
          'Valid campaign ID is missing.'
        );
        return Response.send(res);
      }

      const campaign =
        req.campaign || (await CampaignService.getCampaignById(campaignId));
      ...
      if (
        await CampaignService.campaignBeneficiaryExists(
          campaignId,
          beneficiaryId
        )
      ) {
      ...
      Response.setError(
        ...
        'User is not a campaign beneficiary.'
      );
      return Response.send(res);
    } catch (error) {
      Response.setError(
  ...
  static async NotCampaignBeneficiary(req, res, next) {
    try {
      ...
      const beneficiary = await CampaignService.campaignBeneficiaryExists(
        campaignId,
        beneficiaryId
      );
  ...
  static async BeneficiaryExists(req, res, next) {
    try {
      const organisationId =
        ...
        null;
      const id =
        req.params.beneficiary_id || req.body.beneficiary_id || req.params.id;
      ...
      if (!beneficiary) {
        Response.setError(
          HttpStatusCode.STATUS_RESOURCE_NOT_FOUND,
          'Organisation beneficiary not found.'
        );
        return Response.send(res);
      }
